...
sp_cuarray_var scan(F& fn,
                    Seq& x) {
    sp_cuarray_var result_ary = make_remote<Seq::value_type>(x.size());
    stored_sequence<T> result = get_remote_w<Seq::value_type>(result_ary);
