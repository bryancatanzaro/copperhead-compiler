...
rewriter::result_type rewriter::operator()(const tuple &n) {
    start_match();
    vector<shared_ptr<const expression> > n_values;
...
rewriter::result_type rewriter::operator()(const apply &n) {
    auto n_fn = static_pointer_cast<const name>(
        boost::apply_visitor(
            *this, n.fn()));
    auto n_args = static_pointer_cast<const tuple>(
        (*this)(n.args()));
    start_match();
...
rewriter::result_type rewriter::operator()(const lambda &n) {
    auto n_args = static_pointer_cast<const tuple>(
        (*this)(n.args()));
    auto n_body = static_pointer_cast<const expression>(
...
rewriter::result_type rewriter::operator()(const subscript &n) {
    auto n_src = static_pointer_cast<const name>(
        (*this)(n.src()));
    auto n_idx = static_pointer_cast<const expression>(
...
rewriter::result_type rewriter::operator()(const conditional &n) {
    auto n_cond = static_pointer_cast<const expression>(
        boost::apply_visitor(*this, n.cond()));
    auto n_then = static_pointer_cast<const suite>(
        boost::apply_visitor(*this, n.then()));
    auto n_orelse = static_pointer_cast<const suite>(
...
rewriter::result_type rewriter::operator()(const ret &n) {
    auto n_val = static_pointer_cast<const expression>(
...
rewriter::result_type rewriter::operator()(const bind &n) {
    auto n_lhs = static_pointer_cast<const expression>(
        boost::apply_visitor(*this, n.lhs()));
    auto n_rhs = static_pointer_cast<const expression>(
...
rewriter::result_type rewriter::operator()(const call &n) {
    auto n_sub = static_pointer_cast<const apply>(
...
rewriter::result_type rewriter::operator()(const procedure &n) {
    auto n_id = static_pointer_cast<const name>((*this)(n.id()));
    auto n_args = static_pointer_cast<const tuple>((*this)(n.args()));
    auto n_stmts = static_pointer_cast<const suite>((*this)(n.stmts()));
    start_match();
...
rewriter::result_type rewriter::operator()(const suite &n) {
    start_match();
    vector<shared_ptr<const statement> > n_stmts;
    for(auto i = n.begin(); i != n.end(); i++) {
        auto n_stmt = static_pointer_cast<const statement>(boost::apply_visitor(*this, *i));
...
rewriter::result_type rewriter::operator()(const structure &n) {
    ...
    vector<shared_ptr<const ctype::type_t> > new_typevars;
    for(auto i = n.begin();
        i != n.end();
        i++) {
